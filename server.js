...
function calculateCompatibility(userA, userB) {
  ...
  if (userA.ambitions === userB.ambitions) score += 5;
  else score -= 5;
  ...
  if (partnerFriendMatch[userA.partner_preference]?.includes(userB.friend_description)) score += 15;
  if (partnerFriendMatch[userB.partner_preference]?.includes(userA.friend_description)) score += 15;
  ...
  if (userB.age_preference === "older" && ageA > ageB) score += 10;
  if (userB.age_preference === "younger" && ageA < ageB) score += 10;
  ...
  if (userA.relationship_type === userB.relationship_type) score += 10;
  else score -= 10;
...
app.post("/submit-quiz", (req, res) => {
  ...
  db.run(
    ...
    [
      data.username, data.name, data.age, data.height, data.personality,
      data.ambitions, data.partner_preference, data.zodiac, data.friend_description,
      data.height_preference, data.age_preference, data.love_language,
      data.kids_preference, data.first_date, data.relationship_type,
      data.insecurity, data.flaw, data.dealbreaker
    ],
    function (err) {
...
app.get("/match", (req, res) => {
  ...
  db.all(`SELECT * FROM quiz_answers`, (err, users) => {
    ...
    if (!currentUser) return res.json({ match: null, score: 0 });
...
app.get("/admin/quiz", (req, res) => {
  const sql = `
    SELECT users.username, quiz_answers.*
    FROM users
    JOIN quiz_answers ON users.username = quiz_answers.username;
  `;
